...
class ModelUserUser extends Model {
	...
	public function getUserByGroupId($user_group_id){
		$query = $this->db->query("SELECT user_id,CONCAT(firstname, ' ', lastname) as name FROM `" . DB_PREFIX . "user` WHERE user_group_id = '" . (int)$user_group_id . "'");

		return $query->rows;
	}
	public function getUserAdmin($id){
		$query = $this->db->query("SELECT user_id,CONCAT(firstname, ' ', lastname) as name FROM `" . DB_PREFIX . "user` WHERE user_group_id = 1 and user_id=".(int)$id);
